...
Given the ARDL model, can we find the cointegration vector using a simple Z-transform technique?
...
Applying the $\mathcal{Z}$-transform to @eq-ardl_model, we obtain @eq-z_transformed. To find the long-run (steady-state) relationship, we must evaluate the system's response at zero frequency, which corresponds to setting $Z=1$.

$$
\begin{align}
LRM\left(Z\right)=&+2.62\\&+0.32\cdot LRM\left(Z\right)\cdot Z^{-1}+0.53\cdot LRM\left(Z\right)\cdot Z^{-2}-0.27\cdot LRM\left(Z\right)\cdot Z^{-3}\\&+0.67\cdot LRY\left(Z\right)-0.26\cdot LRY\left(Z\right)\cdot Z^{-1}\\&-1.08\cdot IBO\left(Z\right)-0.11\cdot IBO\left(Z\right)\cdot Z^{-1}+0.29\cdot IBO\left(Z\right)\cdot Z^{-2}\\&+0.13\cdot IDE\left(Z\right)-0.33\cdot IDE\left(Z\right)\cdot Z^{-1}+1.40\cdot IDE\left(Z\right)\cdot Z^{-2}
\end{align}
$$ {#eq-z_transformed}

Setting $Z=1$ in @eq-z_transformed allows us to solve for the long-run multipliers because $Z=1$ represents the DC or steady-state component of the signal. The formula for the long-run multipliers is given in @eq-longrun-multiplier:
...
As shown, this result from the Z-transform method is identical to the cointegration vector produced by the `multipliers()` function in @eq-coint_vector. This confirms that setting $Z=1$ is the correct approach for deriving the long-run equilibrium relationship from the ARDL model's difference equation.
