...
subtitle: "A Formal Proof using the Z-Transform and the Final Value Theorem"
author:
...
This document provides a formal proof for deriving the long-run multipliers from an Autoregressive Distributed Lag (ARDL) model. These multipliers form the cointegration vector, which represents the long-run equilibrium relationship between the variables. The proof will use Z-transform techniques, specifically the Final Value Theorem.

A general ARDL(p, q) model can be expressed as a difference equation:
...
# Z-Transform of the ARDL Equation

To analyze the long-run behavior of the system, we first apply the Z-transform to the ARDL equation (@eq-ardl-general). The Z-transform converts the difference equation into an algebraic equation in the Z-domain. For this analysis, the constant intercept, $\alpha$, is treated as a causal step function input, $\alpha \cdot u(t)$, which is constant for $t \ge 0$. Its Z-transform is $\frac{\alpha}{1-z^{-1}}$. Using this and the time-shifting property of the Z-transform, which states that $\mathcal{Z}\{f_{t-k}\} = z^{-k}F(z)$, we get:
...
\textbf{Term} & \textbf{Description} \\
\hline
Y(z) & \text{The Z-transform of the endogenous variable } Y_t. \\
X(z) & \text{The Z-transform of the exogenous variable } X_t. \\
E(z) & \text{The Z-transform of the error term } \epsilon_t. \\
...
The Final Value Theorem (FVT) in Z-domain analysis allows us to find the steady-state value of a signal directly from its Z-transform. The theorem states that if the system is stable, the final value of the time-domain signal $y_t$ as $t \to \infty$ is found by evaluating the limit as $z \to 1$. This point on the Z-plane corresponds to the DC or steady-state component of the signal, as illustrated in @fig-zplane.

![The Z-Plane, showing the unit circle. The point $z=1$ is highlighted, as this is the point evaluated when applying the Final Value Theorem to find the long-run steady-state gain.](z_plane.svg){#fig-zplane}

The Final Value Theorem is formally stated in @eq-fvt. It allows us to find the steady-state value of a time-domain signal directly from its Z-transform.

$$
\lim_{t \to \infty} y_t = \lim_{z \to 1} (1 - z^{-1}) Y(z)
$$ {#eq-fvt}

To find the long-run multiplier for the exogenous variable $X$, we analyze the system's response to a unit step input, $X_t = u(t)$, where $u(t) = 1$ for $t \ge 0$. The Z-transform of a unit step is $X(z) = \frac{1}{1-z^{-1}}$. The component of the output that is driven by $X$ is $Y_x(z) = H(z)X(z)$. Applying the FVT to this component gives the long-run effect:

$$
\text{Long-Run Multiplier} = \lim_{z \to 1} (1 - z^{-1}) Y_x(z) = \lim_{z \to 1} (1 - z^{-1}) H(z) X(z)
$$

Substituting $X(z) = \frac{1}{1-z^{-1}}$:

$$
\text{Long-Run Multiplier} = \lim_{z \to 1} (1 - z^{-1}) H(z) \frac{1}{1-z^{-1}} = \lim_{z \to 1} H(z)
$$
...
$$
\text{Long-Run Intercept} = \lim_{z \to 1} (1 - z^{-1}) \left( \frac{\frac{\alpha}{1-z^{-1}}}{1 - \sum_{i=1}^{p} \phi_i z^{-i}} \right) = \frac{\alpha}{1 - \sum_{i=1}^{p} \phi_i}
...
Let's apply this technique to the `ardl_3132` model from the `ARDL Experiments.qmd` file. The model equation is:
...
Transforming to the Z-domain and setting $z=1$ (which is equivalent to summing the coefficients for each variable), we can find the long-run multipliers.
...
$$
\text{Intercept Multiplier} = \frac{2.62}{0.4169} = 6.28
$$
$$
\text{LRY Multiplier} = \frac{0.4154}{0.4169} = 0.996
$$
$$
\text{IBO Multiplier} = \frac{-1.8917}{0.4169} = -4.54
$$
$$
\text{IDE Multiplier} = \frac{1.2054}{0.4169} = 2.89
$$
